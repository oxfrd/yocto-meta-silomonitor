...
std::vector<std::string> RealSensorProvider::globSearch(const std::string& pattern) {
...
std::optional<float> RealSensorProvider::readTemp(const std::string& sensorId) {
    try {
...
RealSensorProvider::RealSensorProvider(const std::string& path)
    : sensorPath(path) {}
...
std::map<std::string, float> RealSensorProvider::getTemps() {
